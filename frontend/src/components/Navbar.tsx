...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 shrink-0 bg-[#111116] h-16 px-5 flex items-center justify-between shadow-[0_1px_0_rgba(255,255,255,0.08),0_4px_24px_rgba(0,0,0,0.5)]" style={{ borderBottom: "1px solid rgba(99,102,241,0.25)" }}>
      {/* Logo + Nav */}
        ...
        {user && (
          <>
            <Link href="/profile" className="flex items-center gap-2 group">
              <div className="w-6 h-6 rounded-full bg-indigo-500/20 group-hover:bg-indigo-500/30 flex items-center justify-center transition-colors">
                <span className="text-[10px] font-semibold text-indigo-300">
                  {user.name.split(" ").map((n) => n[0]).slice(0, 2).join("")}
                </span>
              </div>
              <span className="text-xs text-gray-400 group-hover:text-gray-200 font-medium transition-colors">{user.name.split(" ")[0]}</span>
            </Link>
            <div className="w-px h-4 bg-white/10" />
            <button
